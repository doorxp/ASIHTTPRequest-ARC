...
- (void)testAddingRequestsToQueueWhileInProgress
{
	...
	int i;
	for (i=0; i<5; i++) {
		NSURL *url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/the_great_american_novel_(abridged).txt"];
		ASIHTTPRequest *request = [[ASIHTTPRequest alloc] initWithURL:url];
...
- (void)testUploadProgress
{
	...
	int i;
	for (i=0; i<3; i++) {
		NSData *data = [[NSMutableData alloc] initWithLength:fileSizes[i]*1024];
...
- (void)testFailure
{
	...
	NSURL *url;	
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/first"];
	ASIHTTPRequest *request1 = [[ASIHTTPRequest alloc] initWithURL:url];
	[networkQueue addOperation:request1];
	
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/second"];
	ASIHTTPRequest *request2 = [[ASIHTTPRequest alloc] initWithURL:url];
	[networkQueue addOperation:request2];
	
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/third"];
	ASIHTTPRequest *request3 = [[ASIHTTPRequest alloc] initWithURL:url];
	...
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/broken"];
	ASIHTTPRequest *request5 = [[ASIHTTPRequest alloc] initWithURL:url];
...
- (void)testProgressWithAuthentication
{
	...
	NSURL *url;	
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com/ASIHTTPRequest/tests/basic-authentication"];
	ASIHTTPRequest *request = [[ASIHTTPRequest alloc] initWithURL:url];
...
- (void)testWithNoListener
{
	...
	NSURL *url;	
	url = [[NSURL alloc] initWithString:@"http://allseeing-i.com:9999/i/logo.png"];
	requestThatShouldFail = [[ASIHTTPRequest alloc] initWithURL:url];
	[networkQueue addOperation:requestThatShouldFail];
...
- (void)testPartialResume
{
	...
	request = [[ASIHTTPRequest alloc] initWithURL:downloadURL];
	[request setDownloadDestinationPath:downloadPath];
	[request setTemporaryFileDownloadPath:temporaryPath];
	[request setAllowResumeForFileDownloads:YES];
...
- (void)testImmediateCancel
{
	...
	self setImmediateCancelQueue:[[[NSOperationQueue alloc] init]];
	int i;
	for (i=0; i<10; i++) {
		ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"http://allseeing-i.com"]];
		[request setDelegate:self];
...
- (void)testMultipleUploadsThrottlingBandwidth
{
	...
	NSData *data = [[NSMutableData alloc] initWithLength:16*1024];
