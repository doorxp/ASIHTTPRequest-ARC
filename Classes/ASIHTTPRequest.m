...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL willAskDelegateForCredentials;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL willAskDelegateForProxyCredentials;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL shouldTimeOut;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL willRedirect;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL willAskDelegateToConfirmRedirect;

+ (void)performInvocation:(NSInvocation *)invocation onTarget:(id __strong*)target releasingObject:(id)objectToRelease;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL configureProxies;
- (void)fetchPACFile;
...
- (void)main
{
	@try {
		...
		for (header in [self requestHeaders]) {
			CFHTTPMessageSetHeaderFieldValue(request, (__bridge CFStringRef)header, (__bridge CFStringRef)[self requestHeaders][header]);
		}
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					if (credentials[@"Authentication"]) {
						...
						if (CFHTTPMessageApplyCredentialDictionary(request, (__bridge CFHTTPAuthenticationRef)credentials[@"Authentication"], (__bridge CFDictionaryRef)credentials[@"Credentials"], NULL)) {
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			...
			[self setReadStream:(__bridge_transfer NSInputStream *)(CFReadStreamCreateForHTTPRequest(kCFAllocatorDefault, request))];
		}
	}
...
//                                      @YES, kCFStreamSSLAllowsAnyRoot,
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo][@"stream"]) {
			oldStream = [self connectionInfo][@"stream"];
		...
		if (![self connectionInfo]) {
			...
			[self connectionInfo][@"id"] = @((int)nextConnectionNumberToCreate);
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("ASIStreamID"), (__bridge CFTypeRef)([self connectionInfo][@"id"]));
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
            ...
            NSNumber *a= (__bridge_transfer NSNumber*)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount);
			[self setTotalBytesSent:[a unsignedLongLongValue]];
...
+ (void)performSelector:(SEL)selector onTarget:(id __strong*)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
...
+ (void)updateProgressIndicator:(id __strong*)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	#if TARGET_OS_IPHONE
	...
	if (newCredentials && proxyAuthentication && request) {
		...
		if (CFHTTPMessageApplyCredentialDictionary(request, proxyAuthentication, (__bridge CFMutableDictionaryRef)newCredentials, NULL)) {
			...
			if (useSessionPersistence) {
				...
				sessionProxyCredentials[@"Authentication"] = (__bridge id)proxyAuthentication;
	...
	if (newCredentials && requestAuthentication && request) {
		...
		if (CFHTTPMessageApplyCredentialDictionary(request, requestAuthentication, (__bridge CFMutableDictionaryRef)newCredentials, NULL)) {
			...
			if (useSessionPersistence) {
				...
				NSMutableDictionary *sessionCredentials = [NSMutableDictionary dictionary];
				sessionCredentials[@"Authentication"] = (__bridge id)requestAuthentication;
				sessionCredentials[@"Credentials"] = newCredentials;
				sessionCredentials[@"URL"] = [self url];
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[theRequest setContentLength:(unsigned long long)[[self responseHeaders][@"Content-Length"] longLongValue]];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError *__autoreleasing*)err
{
	NSFileManager *fileManager = [[NSFileManager alloc] init];
...
#else
			NSDictionary *proxySettings = SCDynamicStoreCopyProxies(NULL);
#endif
...
+ (NSMutableArray *)sessionCookies
{
	...
	NSMutableArray *cookies = sessionCookies;
...
+ (void)parseMimeType:(NSString *__autoreleasing*)mimeType andResponseEncoding:(NSStringEncoding *)stringEncoding fromContentType:(NSString *)contentType
{
	if (!contentType) {
		return;
	}
...
{
	startedBlock = aStartedBlock;
}

- (void)setHeadersReceivedBlock:(ASIHeadersBlock)aReceivedBlock
{
	headersReceivedBlock = aReceivedBlock;
}
...
{
	bytesSentBlock = aBytesSentBlock;
}
...
{
	dataReceivedBlock = aReceivedBlock;
}
...
{
	requestRedirectedBlock = aRedirectBlock;
}
#endif
