...
//	ASIHTTPRequest *mainRequest;
	...
	ASIBasicBlock __strong startedBlock;

	//block to execute when headers are received
	ASIHeadersBlock __strong headersReceivedBlock;

	//block to execute when request completes successfully
	ASIBasicBlock __strong completionBlock;

	//block to execute when request fails
	ASIBasicBlock __strong failureBlock;

	//block for when bytes are received
	ASIProgressBlock __strong bytesReceivedBlock;

	//block for when bytes are sent
	ASIProgressBlock __strong bytesSentBlock;

	//block for when download size is incremented
	ASISizeBlock __strong downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	ASISizeBlock __strong uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	ASIDataBlock __strong dataReceivedBlock;

	//block for handling authentication
	ASIBasicBlock __strong authenticationNeededBlock;

	//block for handling proxy authentication
	ASIBasicBlock __strong proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    ASIBasicBlock __strong requestRedirectedBlock;
	#endif
}
...
+ (void)performSelector:(SEL)selector onTarget:(id __strong*)target withObject:(id)object amount:(void *)amount callerToRetain:(id)caller;
