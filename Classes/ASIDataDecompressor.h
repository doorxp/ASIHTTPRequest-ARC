...
- (NSData *)uncompressBytes:(Bytef *)bytes length:(NSUInteger)length error:(NSError *__autoreleasing*)err;

// Convenience method - pass it some deflated data, and you'll get inflated data back
+ (NSData *)uncompressData:(NSData*)compressedData error:(NSError *__autoreleasing*)err;
...
+ (BOOL)uncompressDataFromFile:(NSString *)sourcePath toFile:(NSString *)destinationPath error:(NSError *__autoreleasing*)err;
