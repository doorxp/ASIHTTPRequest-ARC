...
- (NSData *)compressBytes:(Bytef *)bytes length:(NSUInteger)length error:(NSError *__autoreleasing*)err shouldFinish:(BOOL)shouldFinish;

// Convenience method - pass it some data, and you'll get deflated data back
+ (NSData *)compressData:(NSData*)uncompressedData error:(NSError *__autoreleasing*)err;
